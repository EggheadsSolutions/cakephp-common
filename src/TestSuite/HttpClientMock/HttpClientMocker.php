...
class HttpClientMocker
{
     ...
     * @return void
     */
    public static function addSniff(array $element)
    {
     ...
     * Мокаем HTTP запрос
     *
     * @param array|string $url Полная строка, либо массив вида ['урл', ['arg1' => 1, ...]]
     * @param string $method
     ...
     */
    public static function mock(array|string $url, string $method): HttpClientMockerEntity
    {
    ...
    {
        if (count($uriArgs)) {
            $mockedUrl = $url . ((!str_contains($url, '?')) ? '?' : '&') . http_build_query($uriArgs);
        } else {
            $mockedUrl = $url;
        }

        return self::mock($mockedUrl, Message::METHOD_GET);
    }
     ...
     *
     * @param string $url
     * @param string|array<string, int|string|bool|null>|null $expectedPostArgs
     * @return HttpClientMockerEntity
     */
    public static function mockPost(string $url, array|string $expectedPostArgs = null): HttpClientMockerEntity
    {
        $mock = self::mock($url, Message::METHOD_POST);
