...
class HttpClientMockerEntity
{
     ...
     * @var null|mixed
     */
    private mixed $_returnValue = null;
     ...
     * @param string $method
     */
    public function __construct(string $url, string $method = Message::METHOD_GET)
    {
        $this->_url = $url;
        $this->_method = $method;
     ...
     */
    public function expectBody(array|string|null $body): self
    {
        if ($this->_method === Message::METHOD_GET) {
    ...
    public function doAction(Request $request): ?string
    {
        ...
        if ($this->_body === null) {
            if (($request->getMethod() === Message::METHOD_POST) && empty($actualBody)) {
     ...
     * @throws ExpectationFailedException
     */
    private function _processResponse(mixed $response): string
    {
        if ($response === null) {
            $response = '';
